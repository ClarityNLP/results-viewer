    ...
    return step["data_entities"].map((d, d_index) => {
      ...
      if (obj) {
        ...
        return (
          <div key={name}>
            <Button
              color={button_colors[new_level]}
              className="PhenotypeDetailButtons"
      ...
      <div className="container-fluid">
        <div className="row SubjectNavRow">
          <small className="SubjectDetailNavigation">
            <Button outline onClick={e => this.props.navigateSubject(-1, e)}>
              <FaArrowLeft />
            </Button>
            <span className="SubjectNavigationPosition">
              {" "}
              {subject.index + 1}
            </span>
          ...
          <h5 className="SubjectDetailHeader float-lg-right">
            Patient #{subject._id}
          </h5>
        </div>
        <div className="row SubjectDetailBody">
          <div className="col-2">{results_nav_view}</div>
          <div className="col-8">
            <div>{phenotype_results_detail}</div>
          </div>
          <div className="col-2" />
        </div>
      </div>
