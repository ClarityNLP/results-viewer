        ...
        const subjects = this.state.subjects.map(p => {
            return (
                <tr
                    ...
                    key={p._id}
                    onClick={e => this.selectSubject(p, e)}
                >
                    <td>{p._id}</td>
                    <td>{p.count}</td>
                </tr>
            ...
            <div className="column is-full">
                {this.state.subjects.length > 0 ? (
                    this.state.show_subject_list ? (
                        <table className="SubjectTable table is-striped is-fullwidth">
                            <thead>
                                <tr>{header_items}</tr>
                            </thead>
                        ...
                        <SubjectResultDetail
                            ...
                            results={this.state.current_results}
                            idx={this.state.selected_index}
                            total={this.state.subjects.length}
                            navigateSubject={this.navigateSubject}
                        />
                    )
                ) : (
