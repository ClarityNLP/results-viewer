...
class DefineFeatureForm extends React.Component {
  ...
  handleFeatureChange = value => {
    ...
    if (value) {
      const tmp_feature = value.value;

      tmp_name = tmp_feature.name;
    }

    this.setState({
      feature: value,
      featureName: tmp_name
    });
  };
  ...
  renderInputsForAlgorithm = () => {
    ...
    const customDescendantsDiv = (
      <div className='field'>
        <label className='checkbox' check='true'>
          <input
            type='checkbox'
  ...
  render() {
    const { collapse, featureName, featureAlgorithm, feature } = this.state;
              ...
              <div className='field'>
                <Select
                  ...
                  options={features.map(feature => {
                    return { value: feature, label: feature.name };
                  })}
                />
              </div>
            ) : null}
