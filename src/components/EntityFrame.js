...
let getHtmlMarkup = (text, start, end) => {
  ...
  return (
    first + '<span class="full-highlighting">' + keyword + "</span>" + last
  );
};
    ...
    return (
      <div key={data["id"]} className="EntityFrame">
        <div onClick={() => this.showDocument(data)}>
          <div>
            <div className="h5">
              {this.props.nlpql_feature === data["feature"] ? (
                <span> </span>
              ) : (
                <span
                  onClick={() =>
        ...
        <Modal
          isOpen={this.state.report_modal}
          toggle={this.toggle}
        ...
        >
          ...
          <ModalBody>
            <div
              className="ReportTextPreview"
              dangerouslySetInnerHTML={this.state.report_text}
            />
          </ModalBody>
          <ModalFooter>
